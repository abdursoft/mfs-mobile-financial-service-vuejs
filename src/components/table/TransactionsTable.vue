  ...
  <div class="p-6 bg-slate-50 rounded-lg shadow-sm mt-3">
    ...
    <div class="flex flex-col md:flex-row items-center md:items-end gap-3 mb-4">
      ...
      <div class="flex items-center gap-2 flex-1">
        <label class="text-sm">TxID</label>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          ...
          <tr v-if="paginated.length === 0">
            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500">No transactions found.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const raw = ref(transferStore?.transactionSummery?.transactions || []);

watch(() => transferStore?.transactionSummery?.transactions, (newTransactions) => {
