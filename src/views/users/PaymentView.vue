...
async function handleSubmit() {
    try {
        ...
        const res = await transfer.onlinePayment(merchantForm);
...
<template>
    <div>
        <TransferUser title="Recent payment" user-role="merchant"  @setToUser="setToUserPhone"  :transactions="transfer.transactionSummery?.transfer" />
            ...
            <div class="w-full md:w-1/2 rounded-md shadow-md px-2 py-4 bg-white mt-10">
                <!-- register form  -->
                    <div class="my-3 flex flex-col items-center justify-center">
                        <h1 class="text-2xl text-center">Make Payment</h1>
                        <small class="text-italic text-center text-slate-500">Online payment, charge 0.33%</small>
                    </div>
                    <InputField type="number" min="1" label="Amount" icon="mdi:dollar" :msg="formError?.amount?.[0]"
                        v-model="merchantForm.amount" placeholder="150" />
                    <InputField type="text" label="Reference" icon="material-symbols-light:description-outline" :msg="formError?.reference?.[0]"
                        v-model="merchantForm.reference" placeholder="Reference" />
                    <PhoneField v-model="merchantForm.user_phone" label="Merchant Phone" :msg="formError?.user_phone?.[0]"
                        @validate="onInput" />
                    <InputField type="text" min="4" label="Wallet PIN" icon="ic:outline-fiber-pin" :msg="formError?.pin?.[0]"
                        v-model="merchantForm.pin" placeholder="****" />
